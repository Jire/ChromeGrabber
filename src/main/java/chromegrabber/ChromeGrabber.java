...
public class ChromeGrabber {
	...
	@FXML
	private TableView<ChromeAccount> valuesTable;
	@FXML
	private TableColumn<ChromeAccount, String> urlColumn;
	@FXML
	private TableColumn<ChromeAccount, String> userColumn;
	@FXML
	private TableColumn<ChromeAccount, String> passColumn;
	@FXML
	...
	private void onSaveClick() throws IOException {
		...
		if (model.getChromeAccounts().isEmpty()) {
			...
			for (ChromeAccount account : model.getChromeAccounts()
					.get(chosenName.replaceAll("\\[", "").replaceAll("\\]", ""))) {

				writer.write(String.format(formatStr, account.getUrl(),
